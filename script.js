...
function bindButtons() {
    ...
    document.getElementById('start-battle-btn').onclick = () => {
        ...
        battleSlots.forEach(hero => { if(hero) { hero.currentHp = hero.hp; hero.maxHp = hero.hp; hero.lastAttackTime = 0; } });
        renderBattleSlots();
        updateBattleUI();
    ...
    if(googleBtn) googleBtn.onclick = () => { playSound('click'); signInWithPopup(auth, provider).catch(e=>alert(e.message)); };
    ...
    if(bgmToggle) bgmToggle.onchange = (e) => { isBgmOn = e.target.checked; if(isBgmOn) { if(!document.getElementById('battle-screen').classList.contains('hidden')){ audioBattle.play().catch(()=>{}); } else { audioBgm.play().catch(()=>{}); } } else { audioBgm.pause(); audioBattle.pause(); } };
    ...
    if(batchConfirmBtn) batchConfirmBtn.addEventListener('click', async () => { 
        playSound('click'); 
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        currentUser = user; document.getElementById('login-section').style.display='none'; document.getElementById('user-info').style.display='flex'; document.getElementById('user-name').innerText = `玩家：${user.displayName || '未命名'}`;
        await loadUserData(user); document.getElementById('game-ui').classList.remove('hidden'); await calculateTotalPowerOnly(user.uid); loadLeaderboard();
    } else {
        document.getElementById('login-section').style.display='block'; document.getElementById('user-info').style.display='none'; document.getElementById('game-ui').classList.add('hidden');
    }
});
...
async function loadUserData(user) {
    try {
        ...
        if (userSnap.exists()) { const data = userSnap.data(); gems = data.gems || 0; gold = data.gold || 0; } 
        else { gems = 1000; gold = 5000; await setDoc(userRef, { name: user.displayName||"未命名", gems, gold, combatPower: 0, createdAt: new Date() }); }
        updateUIDisplay();
    } catch (e) { console.error("讀取失敗:", e); gems = 1000; gold = 5000; updateUIDisplay(); }
}
...
async function saveCardToCloud(card) {
    ...
    const docRef = await addDoc(collection(db, "inventory"), { name: card.name, rarity: card.rarity, atk: card.atk, hp: card.hp, title: card.title, baseAtk: card.atk, baseHp: card.hp, level: 1, stars: 1, obtainedAt: new Date(), owner: currentUser.uid, id: card.id, attackType: card.attackType });
...
async function loadInventory(uid) { try { const container = document.getElementById('inventory-grid'); container.innerHTML = "讀取中..."; const q = query(collection(db, "inventory"), where("owner", "==", uid)); const querySnapshot = await getDocs(q); allUserCards = []; querySnapshot.forEach((docSnap) => { let data = docSnap.data(); if(!data.baseAtk || !data.attackType) { const baseCard = cardDatabase.find(c => c.id === data.id); if(baseCard) { data.baseAtk = baseCard.atk; data.baseHp = baseCard.hp; data.attackType = baseCard.attackType; } } allUserCards.push({ ...data, docId: docSnap.id }); }); filterInventory('ALL'); } catch(e) { console.warn("背包失敗", e); document.getElementById('inventory-grid').innerHTML = "無法讀取"; } }
async function loadLeaderboard() { try { const listDiv = document.getElementById('leaderboard-list'); const q = query(collection(db, "users"), orderBy("combatPower", "desc"), limit(10)); const querySnapshot = await getDocs(q); listDiv.innerHTML = ""; let rank = 1; if(querySnapshot.empty) { listDiv.innerHTML = "<p style='text-align:center'>暫無排名</p>"; return; } querySnapshot.forEach((doc) => { const data = doc.data(); const row = document.createElement('div'); row.className = 'rank-item'; row.innerHTML = `<span>#${rank} ${data.name || "無名氏"}</span><span>${data.combatPower || 0}</span>`; listDiv.appendChild(row); rank++; }); } catch (e) {} }
...
async function endBattle(isWin) {
    ...
    if(isWin) playSound('reveal'); else playSound('dismantle');
