// 🔥 這是您剛剛提供的最新版 SDK (v12.7.0) 🔥
import { initializeApp } from "https://www.gstatic.com/firebasejs/12.7.0/firebase-app.js";
import { getAnalytics } from "https://www.gstatic.com/firebasejs/12.7.0/firebase-analytics.js";

// 🔥 我們補上遊戲需要的 Firestore (資料庫) 和 Auth (登入) 模組，版本號統一為 12.7.0 🔥
...
try {
    ...
    analytics = getAnalytics(app); // 啟動分析功能
    db = getFirestore(app);        // 啟動資料庫
    auth = getAuth(app);           // 啟動驗證
    provider = new GoogleAuthProvider();
    console.log("✅ Firebase (v12.7.0) 初始化成功！");
} catch (e) {
    console.error("❌ Firebase 初始化失敗:", e);
    alert("⚠️ 資料庫連線失敗！請按 F12 打開 Console 查看錯誤訊息。");
}
...
function bindButtons() {
    ...
    if(googleBtn) googleBtn.onclick = () => { 
        ...
        signInWithPopup(auth, provider).catch(e => {
            console.error(e);
            alert("❌ 登入失敗！\n錯誤: " + e.message + "\n請確認您是否在 Firebase 後台啟用了 Google 登入。");
        }); 
    };
    ...
    document.getElementById('guest-btn').onclick = () => { 
        ...
        signInAnonymously(auth)
            .then(async (res) => { await updateProfile(res.user, { displayName: "神秘客" }); location.reload(); })
