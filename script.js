...
function synthesizeClick() { const osc = audioCtx.createOscillator(); const gain = audioCtx.createGain(); osc.type = 'sine'; osc.frequency.setValueAtTime(800, audioCtx.currentTime); osc.frequency.exponentialRampToValueAtTime(300, audioCtx.currentTime + 0.1); gain.gain.setValueAtTime(sfxVolume * 0.5, audioCtx.currentTime); gain.gain.exponentialRampToValueAtTime(0.01, audioCtx.currentTime + 0.1); osc.connect(gain); gain.connect(audioCtx.destination); osc.start(); osc.stop(audioCtx.currentTime + 0.1); }
function synthesizeDismantle() { const buf = audioCtx.createBuffer(1, audioCtx.sampleRate * 0.5, audioCtx.sampleRate); const data = buf.getChannelData(0); for(let i=0;i<data.length;i++) data[i]=Math.random()*2-1; const src = audioCtx.createBufferSource(); src.buffer = buf; const gain = audioCtx.createGain(); gain.gain.setValueAtTime(sfxVolume*0.8, audioCtx.currentTime); gain.gain.exponentialRampToValueAtTime(0.01, audioCtx.currentTime+0.3); src.connect(gain); gain.connect(audioCtx.destination); src.start(); }
function synthesizeInventory() { const buf = audioCtx.createBuffer(1, audioCtx.sampleRate * 0.3, audioCtx.sampleRate); const data = buf.getChannelData(0); for(let i=0;i<data.length;i++) data[i]=Math.random()*2-1; const src = audioCtx.createBufferSource(); src.buffer = buf; const filter = audioCtx.createBiquadFilter(); filter.type='lowpass'; filter.frequency.value=800; const gain = audioCtx.createGain(); gain.gain.setValueAtTime(0, audioCtx.currentTime); gain.gain.linearRampToValueAtTime(sfxVolume*0.6, audioCtx.currentTime+0.1); gain.gain.linearRampToValueAtTime(0, audioCtx.currentTime+0.3); src.connect(filter); filter.connect(gain); gain.connect(audioCtx.destination); src.start(); }
function synthesizePoison() { const osc = audioCtx.createOscillator(); const gain = audioCtx.createGain(); osc.type = 'sawtooth'; osc.frequency.setValueAtTime(200, audioCtx.currentTime); osc.frequency.linearRampToValueAtTime(50, audioCtx.currentTime + 0.3); gain.gain.setValueAtTime(sfxVolume*0.3, audioCtx.currentTime); gain.gain.exponentialRampToValueAtTime(0.01, audioCtx.currentTime+0.3); osc.connect(gain); gain.connect(audioCtx.destination); osc.start(); osc.stop(audioCtx.currentTime+0.3); }
...
// 🔥 修復：同時包含 title 與 attackType 的正確資料庫 🔥
const cardDatabase = [
    ...
    { id: 21, name: "林肯", rarity: "SR", atk: 1100, hp: 1200, title: "解放者", attackType: 'melee' }, 
    ...
    { id: 26, name: "忍者", rarity: "R", atk: 650, hp: 300, title: "影之軍團", attackType: 'melee' },
...
async function loadInventory(uid) {
    ...
    querySnapshot.forEach((docSnap) => { 
        ...
        // 🔥 自動補全 attackType 防止舊卡片出錯 🔥
        if(!data.baseAtk || !data.attackType) { 
            const baseCard = cardDatabase.find(c => c.id === data.id);
            if(baseCard) { data.baseAtk = baseCard.atk; data.baseHp = baseCard.hp; data.attackType = baseCard.attackType; needsUpdate = true; }
        }
...
async function saveCardToCloud(card) {
    ...
    const docRef = await addDoc(collection(db, "inventory"), { 
        ...
        id: card.id, 
        attackType: card.attackType // 新增欄位
    });
...
async function playGachaAnimation(highestRarity) {
...
// 🔥 修復：傷害跳字 (支援顏色) 🔥
function showDamageText(leftPercent, text, color = '#ffeb3b') {
...
function gameLoop() {
    ...
    if (battleState.phase === 'SPAWNING') {
        ...
        if (enemies.length === 0) {
            ...
            if (battleState.wave < 3) showDamageText(50, "3秒後 下一波...", "#fff");
        }
    }
    ...
    enemies.forEach((enemy, eIndex) => {
        ...
        for(let i = startSlot; i <= endSlot; i++) {
             if (battleSlots[i] && battleSlots[i].currentHp > 0) {
                     ...
                     if (now - enemy.lastAttackTime > 800) { 
                        fireProjectile(enemy.el, document.querySelector(`.defense-slot[data-slot="${i}"]`), 'fireball', () => {
                             if (battleSlots[i] && battleSlots[i].currentHp > 0) {
                                 ...
                                 triggerHeroHit(i);
                                 playSound('poison');
                                 showDamageText(slotPos, `-${dmg}`, '#e74c3c'); // 紅字
                                 renderBattleSlots();
                             }
                        });
                ...
                const attackDist = isRanged ? 40 : 6; // 遠程 40, 近戰 6

                if (enemy.position <= slotPos + attackDist && enemy.position >= slotPos - 5) {
                    if (now - battleSlots[i].lastAttackTime > 2000) { 
                         fireProjectile(document.querySelector(`.defense-slot[data-slot="${i}"]`), enemy.el, isRanged?'arrow':'slash', () => {
                             if(enemy.el) { 
                                 const dmg = battleSlots[i].atk;
                                 enemy.currentHp -= dmg;
                                 playSound('hit'); // 擊中音效
                                 showDamageText(enemy.position, `-${dmg}`, '#f1c40f'); // 黃字
                             }
                         });
                ...
                // 阻擋
                if (enemy.position <= slotPos+6 && enemy.position >= slotPos-5) blocked = true;
             }
        }
        ...
        if (enemy.currentHp <= 0) {
            ...
            updateBattleUI(); showDamageText(enemy.position, `+${50}G`, '#f1c40f'); playSound('dismantle');
        } 
    });
