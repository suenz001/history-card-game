...
// ⚠️⚠️⚠️ 請記得將這裡換成您自己的 Firebase 金鑰！ ⚠️⚠️⚠️
const firebaseConfig = {
  apiKey: "AIzaSyCaLWMEi7wNxeCjUQC86axbRsxLMDWQrq8", // 這是範例，沒換的話登入會失敗
...
// 初始化 Firebase (加了 try-catch 防止直接白畫面)
...
try {
    ...
    alert("⚠️ 資料庫連線失敗，請檢查 firebaseConfig 設定！");
}
...
function bindButtons() {
    ...
    if(googleBtn) googleBtn.onclick = () => { 
        playSound('click'); 
        // 防呆檢查
        if(firebaseConfig.apiKey.startsWith("AIzaSyCaLWME")) {
            alert("❌ 您使用的是範例金鑰，無法登入 Google！\n請去 Firebase Console 複製您自己的設定檔。");
            return;
        }
        signInWithPopup(auth, provider).catch(e => alert("登入錯誤: " + e.message)); 
    };
    ...
    if(batchConfirmBtn) batchConfirmBtn.addEventListener('click', async () => { 
        ...
        try { 
            ...
            console.error("批量分解失敗", e); alert("分解過程中發生錯誤，請重試"); 
        } finally {
...
function resetBattleState() {
    isBattleActive = false; if(gameLoopId) cancelAnimationFrame(gameLoopId); 
    if(isBgmOn) { audioBattle.pause(); audioBgm.play().catch(()=>{}); }
